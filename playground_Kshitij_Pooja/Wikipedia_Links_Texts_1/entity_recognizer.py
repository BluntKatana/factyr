import spacy
import nltk
import Wikipedia_Links_Texts_1.WikipediaAPI
import time
...
class NamedEntityRecognizer:
    ...
    def print_entities(self):
        ...
        for entity in self._entities:
            ...
            print("\n")
            wiki_links.append(entity["wikipedia_hit"]["url"])
        return wiki_links
    ...
    def extract_entities(
        self, text: str, keep_types: list = ["GPE", "PERSON", "ORG"]
    ) -> list:
        """
    ...
    def disambiguate_entities(
        ...
        wikipedia_api = Wikipedia_Links_Texts_1.WikipediaAPI.WikipediaAPI(
            "wikipedia.json.gz"
        )
        entity_names = [entity["name"] for entity in self._entities]

        for entity in self._entities:
            candidates = wikipedia_api.get_candidates_from_title(
                entity["name"], limit=15
            )
            entity["wikipedia_hit"] = {"title": "NO HIT", "url": "NO HIT", "score": 0}

            for i, candidate in enumerate(candidates):
                if (
                    "disambiguation" in candidate["title"].lower()
                    or "list of" in candidate["title"].lower()
                ):
                    continue

                text, url = wikipedia_api.get_text_url_from_pageid(
                    candidate["pageid"], candidate["title"][0].upper()
                )
                text_processed = self.process_text(text, entity["name"])
                title_text_processed = self.process_text(
                ...
                title_similarity = (
                    self.jaccard_similarity(entity["context"], title_text_processed)
                    * boost_title_similarity
                )
                ...
                if (
                    candidate["title"] in entity_names
                    and candidate["title"] != entity["name"]
                ):
                ...
                if entity["type"] != "PERSON":
                    ...
                    if text_nlp.ents:
                        if text_nlp.ents[0].label_ == entity["type"]:
                            similarity += (
                                boost_same_category
                                if candidate["title"] == text_nlp.ents[0].text
                                else boost_same_category / 2
                            )
...
# entity_recognizer.extract_entities("Is Rome the capital of Italy?")
